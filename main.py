...
_steps = [
    ...
    # Promote a model export to "prod" before running this explicitly.
...
def go(config: DictConfig):
    ...
    with tempfile.TemporaryDirectory() as tmp_dir:
        ...
        if "basic_cleaning" in active_steps:
            _ = mlflow.run(
                os.path.join(
                    project_root, "src", "basic_cleaning"
                ),
                "main",
                env_manager="local",
                parameters={
        ...
        if "train_random_forest" in active_steps:
            ...
            # Pass the generated file to the training component.

            _ = mlflow.run(
                os.path.join(
                    project_root, "src", "train_random_forest"
                ),
                "main",
                env_manager="local",
                parameters={
        ...
        if "test_regression_model" in active_steps:
            ...
            _ = mlflow.run(
                f"{components_repository}/test_regression_model",
                "main",
                env_manager="local",
                parameters={
